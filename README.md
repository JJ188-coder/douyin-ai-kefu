...
> 版本：0.3.7（2026-08-19）。买家会话**转人工（allocated_service）后**由 AI 接管自动回复。
> 直接调用页面 IM SDK 收发消息，**不模拟点击、不抓接口、不碰签名**。
> 近期变更：过期重推闸——SDK 把几分钟前的买家消息换 clientId 重推时会隔很久突然回旧问题（抢话/啰嗦的根源），现在平台时间距今超 5 分钟的一律不回，内容指纹去重窗 60s→3 分钟；转人工集中回复——接管后第一条回复把转人工前积压的问题合集一次答完（去重、最多 8 条、30 分钟窗），之后恢复对话式逐条回复；连发合并按提问时间从旧到新排序，prompt 加回答顺序规则（先问先答）与防重复规则（说过的不复述、买家反馈过的事实要尊重，如"电话打不通"就不再叫人打电话）（v0.3.7）；发送前最后一刻再查一次人工静音，流水线期间店主接手则丢弃该条回复，彻底不抢话；买家连发多句合并成一条只回一次；大模型加简短硬规则（只答所问、60 字以内、连发统答）（v0.3.6）；修复 AI 回复 30 分钟后被 SDK 重推（已读回执/重连）误判成人工发言导致误静音漏回买家的问题——发送记录取消 30 分钟过期改为容量上限，回执到达时学习 clientId 永久免疫重推（v0.3.5）；平台智能客服（senderRole=4）独立识别，不再误判人工接管（v0.3.4）；会话门禁按买家 ID 归一化，多人同时咨询各回各的、互不排队（v0.3.3）；LLM 请求带编号防并发串台 + 45s 超时自动重试一次，失败记事件日志不静默丢消息（v0.3.2）；人工接管自动静音防抢答 + AI 答不了通知店主（v0.3.1）。
...
| 发送前再查静音 | ✓ | 流水线（排队/节流/等大模型）可能走几十秒；发送前最后一刻再确认店主没接手，接手则丢弃该条回复，彻底不抢话 |
| 连发合并 | ✓ | 买家趁处理间隙连发多句时合成一条理解，只回一条，不一句一答；合并按提问时间从旧到新排序，先问先答 |
| 过期重推闸 | ✓ | 买家消息平台时间距今超 5 分钟一律视为 SDK 迟到重推，不回（根治隔十几分钟突然回旧问题）；内容指纹去重窗 3 分钟兜底 |
| 转人工集中回复 | ✓ | 以转人工为分界：之前积压的问题在接管后第一条回复里集中一段答完（去重/≤8 条/30 分钟窗），之后恢复对话式逐条回复 |
| 防重复规则 | ✓ | 已告知的信息（电话/位置/价格等）不原样复述；买家明确反馈过的事实要尊重（如"电话打不通"就不再建议打电话） |
| 简短硬规则 | ✓ | 只答买家所问、不主动扩展、60 字以内；始终生效，不随人设覆盖 |
...
```
cd plugin/tests
node agent-gate.test.mjs               # 门禁 26 项（历史/双推/角色/指纹/锁/人工静音/发送前再查静音/连发合并/过期重推/乱序合并升序/转人工集中回复/needsHuman/关闭重开/跨买家隔离/两种 convId 格式分叉/人设包装）
