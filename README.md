> 本仓库为「抖音来客 AI 人工接管助手」独立拆分仓；原混合仓库仅作回退备份。真实 API Key、飞书凭据、浏览器登录 profile 与运行数据不入库；如需交接配置，只使用 docs/handover-config.example.json 模板重新填写。

# 抖音来客 AI 人工接管助手 — 使用与维护

> 版本：0.4.2（2026-08-20）。买家会话**转人工（allocated_service）后**由 AI 接管自动回复。
> 直接调用页面 IM SDK 收发消息，**不模拟点击、不抓接口、不碰签名**。
> 近期变更：联系方式策略入库——顾客要电话时先引导加 VX（私域沉淀）再附电话，必说"下午3-5点高峰期电话基本打不通、周末更难"；转办承诺上报区分方向——"店家主动"（让同事加您VX/给您回电）才推飞书，"引导买家自己加 VX"不推（防刷屏）（v0.4.2）；转办承诺上报——AI 承诺了要人办的事则推飞书+红角标提醒店主落实，不静音（v0.4.1）；反幻觉门禁降敏修复——全角冒号啃价格/中文数字误杀"一下"/金额与数量分级核对、合计计算放行（v0.4.0）；反幻觉门禁——prompt 软约束 + 发送前程序硬闸（行动承诺闸 + 事实核对闸，证据排除 AI 自己防自我循环），拦截后自动替换为安全兜底 + needsHuman 通知店主（v0.3.9，事故：AI 向投诉蚊子的买家承诺"拿药膏送到10号桌"）；口吻铁律+情绪判断+吐槽应对+买家不满预警+禁波浪线（v0.3.8）；过期重推闸——SDK 把几分钟前的买家消息换 clientId 重推时会隔很久突然回旧问题（抢话/啰嗦的根源），现在平台时间距今超 5 分钟的一律不回，内容指纹去重窗 60s→3 分钟；转人工集中回复——接管后第一条回复把转人工前积压的问题合集一次答完（去重、最多 8 条、30 分钟窗），之后恢复对话式逐条回复；连发合并按提问时间从旧到新排序，prompt 加回答顺序规则（先问先答）与防重复规则（说过的不复述、买家反馈过的事实要尊重，如"电话打不通"就不再叫人打电话）（v0.3.7）；发送前最后一刻再查一次人工静音，流水线期间店主接手则丢弃该条回复，彻底不抢话；买家连发多句合并成一条只回一次；大模型加简短硬规则（只答所问、60 字以内、连发统答）（v0.3.6）；修复 AI 回复 30 分钟后被 SDK 重推（已读回执/重连）误判成人工发言导致误静音漏回买家的问题——发送记录取消 30 分钟过期改为容量上限，回执到达时学习 clientId 永久免疫重推（v0.3.5）；平台智能客服（senderRole=4）独立识别，不再误判人工接管（v0.3.4）；会话门禁按买家 ID 归一化，多人同时咨询各回各的、互不排队（v0.3.3）；LLM 请求带编号防并发串台 + 45s 超时自动重试一次，失败记事件日志不静默丢消息（v0.3.2）；人工接管自动静音防抢答 + AI 答不了通知店主（v0.3.1）。
...
| 口吻铁律 | ✓ | 永远用商家对顾客的口吻；纯表情/无内容消息只回一句中性礼貌话术，不发挥 |
| 情绪判断 | ✓ | 回复前先判断买家情绪：带不满信号立刻收敛（禁"哈哈"等轻佻尾巴），拿不准一律按不高兴处理 |
| 吐槽应对+预警 | ✓ | 买家抱怨先诚恳道歉并表示改进；命中不满信号同时给店主发红角标+飞书提醒（AI 不闭嘴，回复照走） |
| 禁波浪线 | ✓ | 回复里禁止出现 ～/〜/~：prompt 规则约束 + 发送前清洗直接删除，双保险 |
...
```
cd plugin/tests
node agent-gate.test.mjs               # 门禁 27 项（历史/双推/角色/指纹/锁/人工静音/发送前再查静音/连发合并/过期重推/乱序合并升序/转人工集中回复/买家不满预警/needsHuman/关闭重开/跨买家隔离/两种 convId 格式分叉/人设包装）
...
node anti-hallucination.test.mjs       # 反幻觉门禁 15 项（送物/派人/预留/退款赔偿拦截、知识库事实放行、买家回声放行、AI 自言不作证据、12/24 小时等价）
```
