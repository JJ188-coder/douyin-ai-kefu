...
await sleep(50);
console.assert(pendingBg.length === 2, '❌ 应有 2 个在途请求, 实际 ' + pendingBg.length);
pendingBg[1].cb({ ok: true, text: 'B的回复：可以预约' });   // B 先回来
pendingBg[0].cb({ ok: true, text: 'A的回复：398元' });      // A 后回来
const [rA, rB] = await Promise.all([pA, pB]);
console.assert(rA === 'A的回复：398元', '❌ A 拿错了回复: ' + rA);
console.assert(rB === 'B的回复：可以预约', '❌ B 拿错了回复（串台）: ' + rB);
...
console.assert(mainListeners.size === 0, '❌ 请求完成后监听器应已清理, 残留 ' + mainListeners.size);
...
console.assert(d && d.reply === '重试后的回复', '❌ 重试后应拿到回复, 实际 ' + (d && d.reply));
