const PORT=9223; const DURATION=Number(process.env.DURATION||300);
(async()=>{ const start=Date.now(); const seen=new Set();
 while(Date.now()-start<DURATION*1000){ try{
   ...
   const r=await send('Runtime.evaluate',{expression:'JSON.stringify(window.__s5||[])',returnByValue:true});
   const arr=r.result?.result?.value?JSON.parse(r.result.result.value):[];
   for(const it of arr){const k=JSON.stringify(it); if(!seen.has(k)){seen.add(k);console.log(new Date().toISOString().slice(11,19),JSON.stringify(it));}}
   ws.close(); }catch(e){}
   await new Promise(r=>setTimeout(r,2000));
 } console.log('POLL_DONE total=',seen.size); process.exit(0); })();
